...
def validate_repeats_input():
    while True:
        repeats = input("Введите количество жалоб: ")
        try:
            repeats = int(repeats)
            if repeats <= 0:
                print(colored("Число должно быть положительным!", 'red'))
                continue
            return repeats
        except ValueError:
            print(colored("Пожалуйста, введите целое число!", 'red'))
...
def get_complaint_text(complaint_choice, username, telegram_id):
    complaints = {
        ...
        "3": f"Здраствуйте. Аккаунт {username}, id {telegram_id} оскорбляет меня и мою маму. Мне это очень не приятно, поэтому и пишу вам. Огромная прозьба разобраться и заблокировать данного пользователя т.к это нарушает политику сервиса. Благодарю",
    ...
    return complaints.get(complaint_choice, "Некорректная причина жалобы")
...
def send_complaint(username, telegram_id, number, email, repeats, complaint_choice, proxies=None):
    ...
    complaints_sent = 0
    text = get_complaint_text(complaint_choice, username, telegram_id)
...
def show_menu():
    ...
    while True:
        choice = input(colored("Введите номер: ", "magenta"))
        if choice in ["1", "2", "3", "4", "5", "6", "7", "8"]:
            return choice
        print(colored("Некорректный ввод. Пожалуйста, выберите от 1 до 8", "red"))

def main():
    check_password()
    
    complaint_choice = show_menu()
    
    username = input("Введите @username: ")
    telegram_id = input("Введите Telegram ID: ")
    repeats = validate_repeats_input()
    
    if complaint_choice == "5":
        number = input("Введите номер телефона аккаунта: ")
    else:
        number = generate_phone_number()
    
    email = generate_random_email()
    proxies = {'http': random.choice(get_proxies_list())}
